...
class strLabelConverter(object):
    ...
    def __init__(self, alphabet, ignore_case=False): # 把字母表内所有字符与编码一一对应
        self._ignore_case = ignore_case
        if self._ignore_case: # 若忽略大小写，把所有字母都转为小写处理
    ...
    def encode(self, text):
        ...
        try:
            if isinstance(text, str):
                ...
                length = [len(text)]
            elif isinstance(text, collections.Iterable): # 若是多段str的列表
                length = [len(s) for s in text]
                text = ''.join(text) # 先合并
                text, _ = self.encode(text) # 再调用encode转换
        except KeyError as e: # 出现字典中不存在的字符，引起报错
            # print(text)
            print(e)
            for ch in text:
